---
title: "Spline Fitting All Hives Individually Over 1 Day, Simulating Missing Data for in_count & out_count"
...
#  lines(spline_fit, col = "red", lwd = 2)
  ...
  # plot(one_hive_one_day$weight_timestamp_numeric, one_hive_one_day$in_count_diff, pch = 16)
  ...
  for (end_time in missing_end_times) {
    ...
    if (timestamps_remaining >= 4) {
      ...
      # timestamps_remaining <- length(one_hive_one_day$weight_timestamp >= missing_start & one_hive_one_day$weight_timestamp <= end_time)
      # timestamps_remaining <- (length(one_hive_one_day$in_count_diff_original) - length(one_hive_one_day$in_count_diff_imputed))
      # TODO: try this version: timestamps_remaining <- one_hive_one_day %>% filter(weight_timestamp >= missing_start & weight_timestamp <= end_time) %>% count(weight_timestamp)
      #####################repeating code chunk lines 139-193 to plot for out_count:
      ...
      # Compare imputed data with actual data (before removal): out_count
      # Keep a copy of the original data before simulating missing data
